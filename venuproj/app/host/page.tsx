...
export default function HostPage() {
  ...
  // Set default selected listing when listings are loaded
  useEffect(() => {
  ...
  // Load blocked dates for the selected listing
  useEffect(() => {
    if (selectedListingId && user) {
      const blockedDatesKey = `blockedDates_${selectedListingId}`;
  ...
  // Listen for blocked dates updates from other tabs/windows
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key && e.key.startsWith('blockedDates_') && selectedListingId) {
        const listingIdFromKey = e.key.replace('blockedDates_', '');
        if (listingIdFromKey === selectedListingId && e.newValue) {
          try {
            ...
            console.error('Error parsing blocked dates from storage event:', error);
          }
        }
      }
    };
    ...
    const handleCustomEvent = (e: CustomEvent) => {
      if (e.detail?.listingId === selectedListingId) {
        setBlockedDates(e.detail.blockedDates || []);
      }
    };

    window.addEventListener('storage', handleStorageChange);
    window.addEventListener('blockedDatesUpdated', handleCustomEvent as EventListener);

    return () => {
      window.removeEventListener('storage', handleStorageChange);
      window.removeEventListener('blockedDatesUpdated', handleCustomEvent as EventListener);
    };
  }, [selectedListingId]);
  ...
  const handleBlockToggle = () => {
    ...
    // Save blocked dates to localStorage for the selected listing only
    const blockedDatesKey = `blockedDates_${selectedListingId}`;
    ...
    window.dispatchEvent(new CustomEvent('blockedDatesUpdated', {
      detail: { listingId: selectedListingId, blockedDates: updatedBlockedDates }
    }));
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            gap: '12px',
                            padding: '12px',
                            border: `1px solid ${isSelected ? '#1976d2' : '#e6e6e6'}`,
                            borderRadius: '8px',
                        ...
                        >
                          <div style={{
                            ...
                            backgroundColor: mainPhoto?.url ? 'transparent' : '#e6e6e6',
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{
                              ...
                              {listing.propertyName || 'Unnamed Listing'}
                            </div>
                            <div style={{
                              fontSize: '12px',
                              color: '#666',
          ...
          alignItems: 'center',
            ...
            <button
              ...
              <div style={{
                ...
                backgroundColor: '#fff',
                position: 'relative',
                opacity: selectedListingId ? 1 : 0.6
              }}>
                {!selectedListingId && (
                  <div style={{
                    position: 'absolute',
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        Select a listing to manage dates
                      </div>
                      <div style={{
                        fontSize: '14px',
                        color: '#666'
                      }}>
                        Choose a listing from the left sidebar to block or unblock dates
                      </div>
                    </div>
                  </div>
                )}
