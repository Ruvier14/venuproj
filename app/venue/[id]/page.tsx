...
export default function VenueDetails() {
  ...
  const handleDateClick = (day: number, month: number, year: number, isBooking: boolean = false) => {
    const date = new Date(year, month, day);
    setSelectedDate(date);
    
    if (isBooking) {
      // Update booking date display
      const bookingDateElement = document.getElementById("booking-date-display");
  ...
  const photoGallery: PhotoItem[] = [
    { id: '1', url: venue?.image || '/api/placeholder/800/600', caption: venue?.name || 'Insert Event Venue', category: 'exterior' },
    { id: '2', url: '/api/placeholder/400/300', caption: 'Side Porch', category: 'outdoor' },
    { id: '3', url: '/api/placeholder/400/300', caption: 'Front Entrance', category: 'entrance' },
    { id: '4', url: '/api/placeholder/400/300', caption: 'Event Space', category: 'event-space' },
    { id: '5', url: '/api/placeholder/400/300', caption: 'Lobby Reception', category: 'lobby' },
    { id: '6', url: '/api/placeholder/400/300', caption: 'Main Space', category: 'main-space' },
    { id: '7', url: '/api/placeholder/400/300', caption: 'Secondary Space', category: 'secondary-space' },
    { id: '8', url: '/api/placeholder/400/300', caption: 'Outdoor Area', category: 'outdoor' },
  ];

  const photoCategories = [
    { id: 'all', label: 'All photos', count: 0 },
    { id: 'event-space', label: 'Event space', count: photoGallery.filter(p => p.category === 'event-space').length },
    { id: 'exterior', label: 'Exterior', count: photoGallery.filter(p => p.category === 'exterior').length },
    { id: 'entrance', label: 'Entrance', count: photoGallery.filter(p => p.category === 'entrance').length },
    { id: 'lobby', label: 'Lobby/reception', count: photoGallery.filter(p => p.category === 'lobby').length },
    { id: 'main-space', label: 'Main space', count: photoGallery.filter(p => p.category === 'main-space').length },
    { id: 'secondary-space', label: 'Secondary space', count: photoGallery.filter(p => p.category === 'secondary-space').length },
    { id: 'outdoor', label: 'Outdoor area', count: photoGallery.filter(p => p.category === 'outdoor').length },
  ];
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (bookingCalendarRef.current && !bookingCalendarRef.current.contains(event.target as Node)) {
        setBookingDateOpen(false);
      }
      if (bookingOccasionRef.current && !bookingOccasionRef.current.contains(event.target as Node)) {
        setBookingOccasionOpen(false);
      }
      if (bookingGuestRef.current && !bookingGuestRef.current.contains(event.target as Node)) {
  ...
  useEffect(() => {
    ...
    if (!selectedOccasion) {
      ...
      if (occasionParam) {
        ...
        const occasionOptions = dropdownOptions.occasion.map(opt => opt.title);
        // Try exact match first
        let matchedOption = occasionOptions.find(opt => opt === occasionParam);
        if (!matchedOption) {
          // Try partial match
          matchedOption = occasionOptions.find(opt => 
    ...
    if (!selectedGuest) {
      ...
      if (guestParam) {
        ...
        if (!matchedOption) {
          // Try partial match
          matchedOption = guestOptions.find(opt => 
  ...
  return (
    <div className="page-shell">
      ...
      <main style={{ padding: '0', backgroundColor: '#fff', maxWidth: '1760px', margin: '0 auto' }}>
        ...
        <div style={{ width: '100%', marginBottom: '48px', display: 'flex', justifyContent: 'center' }}>
          <div style={{ 
            display: 'grid', 
        ...
        <div style={{ display: 'flex', gap: '80px', padding: '0 80px 48px 80px' }}>
          ...
          <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '24px', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '14px', color: '#1976d2', textDecoration: 'underline', cursor: 'pointer', marginBottom: '8px' }}>
                      See host profile
                    </div>
                    <button
                      type="button"
                ...
                <div style={{
                  ...
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: '24px', fontWeight: '700', color: '#1565c0', marginBottom: '4px' }}>
                      0/10
                    </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
                  {venue.amenities.map((amenity, index) => (
                    <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <span style={{ fontSize: '16px', color: '#222' }}>Insert Amenties</span>
                    </div>
                  ))}
                </div>
            ...
            <div style={{ marginBottom: '32px', paddingBottom: '32px', borderBottom: '1px solid #e6e6e6' }}>
              <div>
                <h2 style={{ fontSize: '22px', fontWeight: '600', color: '#222', marginBottom: '16px' }}>House Rules</h2>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', marginBottom: '24px' }}>
                  <div>
                    <div style={{ fontSize: '16px', color: '#222', marginBottom: '8px' }}>Check in after 3:00 PM</div>
                    <div style={{ fontSize: '16px', color: '#222', marginBottom: '8px' }}>Check out before 11:00 AM</div>
                    <div style={{ marginBottom: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                        <div style={{ position: 'relative', width: '20px', height: '20px' }}>
                          ...
                          <line x1="2" y1="22" x2="22" y2="2" stroke="#ff385c" strokeWidth="2.5" style={{ position: 'absolute', top: 0, left: 0 }}/>
                        </div>
                        <div style={{ fontSize: '16px', fontWeight: '600', color: '#222' }}>Children</div>
                      </div>
                      <div style={{ fontSize: '16px', color: '#222', paddingLeft: '28px' }}>Adults only</div>
                    </div>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                        <div style={{ position: 'relative', width: '20px', height: '20px' }}>
                        ...
                        <div style={{ fontSize: '16px', fontWeight: '600', color: '#222' }}>Pets</div>
                      </div>
                      <div style={{ fontSize: '16px', color: '#222', paddingLeft: '28px' }}>No pets allowed</div>
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: '16px', color: '#222', marginBottom: '20px' }}>Minimum age to rent: 25</div>
                    <div style={{ marginBottom: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                        <div style={{ position: 'relative', width: '20px', height: '20px' }}>
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#222" strokeWidth="2">
                        ...
                        <div style={{ fontSize: '16px', fontWeight: '600', color: '#222' }}>Events</div>
                      </div>
                      <div style={{ fontSize: '16px', color: '#222', paddingLeft: '28px' }}>No events allowed</div>
                    </div>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                        <div style={{ position: 'relative', width: '20px', height: '20px' }}>
                        ...
                        <div style={{ fontSize: '16px', fontWeight: '600', color: '#222' }}>Smoking</div>
                      </div>
                      <div style={{ fontSize: '16px', color: '#222', paddingLeft: '28px' }}>Smoking is not permitted</div>
                    </div>
                  </div>
                </div>
                
                {/* Divider */}
                <div style={{ borderTop: '1px solid #e6e6e6', marginTop: '24px', paddingTop: '24px' }}></div>
                
                {/* Check-out instructions */}
                <div>
                  <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#222', marginBottom: '12px' }}>Check-out instructions</h3>
                  <p style={{ fontSize: '16px', color: '#222', marginBottom: '12px' }}>
                    The host requires you complete the following before checking out:
                  </p>
                  <ul style={{ fontSize: '16px', color: '#222', lineHeight: '1.8', paddingLeft: '20px', marginBottom: '12px' }}>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px', gap: '16px' }}>
                  <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#222', margin: 0 }}>Damage and incidentals</h3>
                  <p style={{ fontSize: '16px', color: '#666', margin: 0, textAlign: 'right' }}>
              ...
              {/* Cancellation */}
              <div>
                <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#222', marginBottom: '24px' }}>Cancellation</h3>
                
                {/* Timeline */}
                <div style={{ backgroundColor: '#f6f7f8', borderRadius: '8px', padding: '32px 24px', marginBottom: '24px' }}>
                  <div style={{ position: 'relative', height: '60px' }}>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                        ...
                        <div style={{ fontSize: '14px', fontWeight: '500', color: '#222', marginTop: '40px' }}>Today</div>
                      </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                        ...
                        <div style={{ fontSize: '14px', fontWeight: '500', color: '#222', marginTop: '40px' }}>Jan 2</div>
                      </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                        ...
                        <div style={{ fontSize: '14px', fontWeight: '500', color: '#222', marginTop: '40px' }}>Jan 9</div>
                      </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                        ...
                        <div style={{ fontSize: '14px', fontWeight: '500', color: '#222', marginTop: '40px' }}>Check-in</div>
                      </div>
                    </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  <div style={{ borderBottom: '1px solid #e6e6e6', paddingBottom: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                      <div style={{ fontSize: '16px', fontWeight: '600', color: '#222' }}>Before Jan 2</div>
                      <div style={{ fontSize: '16px', fontWeight: '600', color: '#222' }}>Full refund</div>
                    </div>
                  ...
                  <div style={{ borderBottom: '1px solid #e6e6e6', paddingBottom: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                      <div style={{ fontSize: '16px', fontWeight: '600', color: '#222' }}>Before Jan 9</div>
                      <div style={{ fontSize: '16px', fontWeight: '600', color: '#222' }}>Partial refund</div>
                    </div>
                    <p style={{ fontSize: '16px', color: '#666', margin: 0 }}>
                  ...
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                      <div style={{ fontSize: '16px', fontWeight: '600', color: '#222' }}>After Jan 9</div>
                      <div style={{ fontSize: '16px', fontWeight: '600', color: '#222' }}>No refund</div>
                    </div>
                    <p style={{ fontSize: '16px', color: '#666', margin: 0 }}>
            ...
            {/* Reviews Section */}
            <div style={{ marginBottom: '32px', paddingBottom: '32px', borderBottom: '1px solid #e6e6e6' }}>
              <h2 style={{ fontSize: '22px', fontWeight: '600', color: '#222', marginBottom: '24px' }}>Reviews</h2>
              ...
              <div style={{ display: 'flex', gap: '16px', position: 'relative' }}>
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: '14px', color: '#222', display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                <div style={{ display: 'flex', gap: '16px', flex: 1, position: 'relative', alignItems: 'center' }}>
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: '18px', fontWeight: '600', color: '#222', marginBottom: '12px' }}>
                      10/10 Excellent
                    </div>
                    <div style={{ fontSize: '16px', color: '#222', lineHeight: '1.6', marginBottom: '8px' }}>
          ...
          <div style={{ width: '400px', position: 'sticky', top: '100px', height: 'fit-content' }}>
            <div style={{
              ...
              <div style={{ marginBottom: '24px', position: 'relative' }}>
                ...
                >
                  <div id="booking-date-display" style={{ fontSize: '14px', color: selectedDate ? '#222' : '#666' }}>
                    {selectedDate ? `${monthNames[selectedDate.getMonth()]} ${selectedDate.getDate()}, ${selectedDate.getFullYear()}` : 'Add date'}
                  </div>
                </div>
                {bookingDateOpen && (
                  <div ref={bookingCalendarRef} className="calendar-dropdown" style={{ position: 'absolute', top: 'calc(100% + 8px)', left: 0, right: 0, zIndex: 2000, marginTop: '4px' }}>
                    <div className="calendar-title">When is your event?</div>
                    <div className="calendar-container">
                      {/* First Calendar */}
                      <div className="calendar-month">
                            ...
                            {renderCalendar(calendarMonth, calendarYear).map((day, index) => {
                              ...
                              return (
                                <button
                                  ...
                                  type="button"
                                  disabled={day === null || isPast}
                                  onClick={() => day !== null && !isPast && handleDateClick(day, calendarMonth, calendarYear, true)}
                                >
                                  {day}
                                </button>
                              );
                              ...
                              return renderCalendar(next.month, next.year).map((day, index) => {
                                ...
                                return (
                                  <button
                                    ...
                                    type="button"
                                    disabled={day === null || isPast}
                                    onClick={() => day !== null && !isPast && handleDateClick(day, next.month, next.year, true)}
                                  >
                                    {day}
                                  </button>
                                );
              ...
              <div style={{ marginBottom: '24px', position: 'relative' }}>
                <div style={{ fontSize: '16px', fontWeight: '600', color: '#222', marginBottom: '8px' }}>Occasion</div>
                <div 
                  style={{
                    padding: '12px',
                    border: '1px solid #e6e6e6',
                ...
                >
                  <div id="booking-occasion-display" style={{ fontSize: '14px', color: selectedOccasion ? '#222' : '#666' }}>
                ...
                {bookingOccasionOpen && (
                  <div ref={bookingOccasionRef} className="guest-dropdown" style={{ position: 'absolute', top: 'calc(100% + 8px)', left: 0, right: 0, zIndex: 2000, marginTop: '4px' }}>
              ...
              <div style={{ marginBottom: '24px', position: 'relative' }}>
                <div style={{ fontSize: '16px', fontWeight: '600', color: '#222', marginBottom: '8px' }}>Guests</div>
                <div 
                  style={{
                    padding: '12px',
                    border: '1px solid #e6e6e6',
                ...
                >
                  <div id="booking-guest-display" style={{ fontSize: '14px', color: selectedGuest ? '#222' : '#666' }}>
                ...
                {bookingGuestOpen && (
                  <div ref={bookingGuestRef} className="guest-dropdown" style={{ position: 'absolute', top: 'calc(100% + 8px)', left: 0, right: 0, zIndex: 2000, marginTop: '4px' }}>
                    <div className="guest-dropdown-title">Number of Guests:</div>
              ...
              <div style={{ marginBottom: '24px' }}>
                <button
                  ...
                  onClick={() => {
                    ...
                    const checkIn = `${monthNames[checkInDate.getMonth()]} ${checkInDate.getDate()}, ${checkInDate.getFullYear()}`;
                    const checkOut = `${monthNames[checkOutDate.getMonth()]} ${checkOutDate.getDate()}, ${checkOutDate.getFullYear()}`;
                    ...
                    const booking = {
                      ...
                      amenities: venue?.amenities && venue.amenities.length > 0 ? venue.amenities : ['FREE Wi-Fi', 'Breakfast'], // Use venue amenities or default
                      specialRequests: '',
                    ...
                    localStorage.setItem(`upcomingBookings_${user.uid}`, JSON.stringify(existingBookings));

                    // Show success message and redirect
                    alert('Booking confirmed! Check "My Events" to view your booking.');
                    router.push('/events');
                  }}
                  style={{
                    width: '100%',
              ...
              <div style={{ paddingTop: '16px', borderTop: '1px solid #e6e6e6', display: 'flex', justifyContent: 'center' }}>
                <button
                  type="button"
                  onClick={() => {
          ...
          >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px', marginBottom: '24px' }}>
              ...
              <button
                type="button"
                onClick={() => {
                  window.location.href = `mailto:?subject=Check out this venue: ${encodeURIComponent(venue.name)}&body=${encodeURIComponent(window.location.href)}`;
                }}
                style={{
                  display: 'flex',
                  flexDirection: 'column',
              ...
              <button
                type="button"
                onClick={() => {
                  window.open(`https://wa.me/?text=${encodeURIComponent(`Check out this venue: ${venue.name} ${window.location.href}`)}`, '_blank');
                }}
                style={{
                  display: 'flex',
                  flexDirection: 'column',
          ...
          >
            <div className="modal-content" style={{ padding: '24px' }}>
                ...
                {photoCategories.map((category) => (
                  <button
                    ...
                    style={{
                      ...
                      border: '1px solid #e6e6e6',
                      borderRadius: '24px',
                      backgroundColor: selectedPhotoCategory === category.id ? '#222' : 'white',
                      color: selectedPhotoCategory === category.id ? 'white' : '#222',
                      fontSize: '14px',
                      fontWeight: '500',
                      cursor: 'pointer',
                      whiteSpace: 'nowrap',
              ...
              <h2 style={{ 
                ...
                {photoCategories.find(c => c.id === selectedPhotoCategory)?.label || 'All photos'}
              </h2>
      ...
      {amenitiesModalOpen && (
        ...
        >
          ...
          >
            ...
            <div className="modal-content" style={{ padding: '24px' }}>
              <h2 style={{ fontSize: '22px', fontWeight: '600', color: '#222', marginBottom: '24px', paddingRight: '40px' }}>Amenities</h2>
              
              <div style={{ maxHeight: 'calc(90vh - 120px)', overflowY: 'auto', paddingRight: '8px' }}>
                {/* Essentials */}
                <div style={{ marginBottom: '24px' }}>
                  <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#222', marginBottom: '16px' }}>Essentials</h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {['Wireless internet', 'WiFi', 'Internet', 'Towels provided', 'Linens provided', 'Air conditioning', 'Hair dryer', 'Shampoo', 'Toilet paper', 'Paper towels', 'Basic soaps', 'Heating'].map((item, index) => (
                      <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#222" strokeWidth="2">
                          <path d="M5 13l4 4L19 7" />
                        </svg>
                        <span style={{ fontSize: '16px', color: '#222' }}>{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Kitchen */}
                <div style={{ marginBottom: '24px' }}>
                  <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#222', marginBottom: '16px' }}>Kitchen</h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {['Kitchen', 'Kitchenette', 'Refrigerator', 'Small refrigerator', 'Microwave', 'Dishes & utensils', 'Coffee maker', 'Toaster', 'Toaster Oven'].map((item, index) => (
                      <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#222" strokeWidth="2">
                          <path d="M5 13l4 4L19 7" />
                        </svg>
                ...
                {/* Outside */}
                <div style={{ marginBottom: '24px' }}>
                  <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#222', marginBottom: '16px' }}>Outside</h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                {/* Entertainment */}
                <div style={{ marginBottom: '24px' }}>
                  <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#222', marginBottom: '16px' }}>Entertainment</h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {['Games', 'Television', 'Satellite / cable'].map((item, index) => (
                      <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#222" strokeWidth="2">
                          <path d="M5 13l4 4L19 7" />
                        </svg>
                ...
                {/* Laundry */}
                <div style={{ marginBottom: '24px' }}>
                  <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#222', marginBottom: '16px' }}>Laundry</h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {['Washing machine', 'Dryer', 'Iron', 'Laundry detergent'].map((item, index) => (
                      <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#222" strokeWidth="2">
                          <path d="M5 13l4 4L19 7" />
                        </svg>
      ...
      {reviewsModalOpen && (
        ...
        >
          ...
          >
            ...
            <div className="modal-content" style={{ padding: '24px' }}>
              <h2 style={{ fontSize: '22px', fontWeight: '600', color: '#222', marginBottom: '16px', paddingRight: '40px' }}>78 reviews</h2>
              
              {/* Tab */}
              <div style={{ borderBottom: '1px solid #e6e6e6', marginBottom: '24px' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                        <div style={{ display: 'flex', gap: '2px' }}>
                          {[...Array(5)].map((_, i) => (
                            <svg key={i} width="16" height="16" viewBox="0 0 24 24" fill="#222" stroke="#222" strokeWidth="0">
                              <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                            </svg>
                          ))}
                        </div>
                        <span style={{ fontSize: '14px', color: '#666' }}>November 2025</span>
                      </div>
                    </div>
                  </div>
                  <div style={{ fontSize: '16px', color: '#222', lineHeight: '1.6' }}>
                    This is my third time in the Philippines every 6 months. This time I was at a resort in Cebu for 3 weeks, then came back to Manila. The resort had no wifi and inconvenient, so I looked at Marvie's condo. Best decision. Marvie made sure I had everything, including a back up preloaded data internet modem. The building is secure, quiet, and in an area that doesn't flood. Power was out for only 24 hours after the typhoon, which is outstanding. Seriously considering buying a 2-bedroom unit in the community. Marvie's studio is super clean, no bugs. Extra reading glasses were there too. The security guy got me a stellar ride to the airport in about 5 mins.
                  </div>
                </div>
                  ...
                  <div style={{ display: 'flex', gap: '16px', marginBottom: '12px' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                        ...
                        <span style={{ fontSize: '14px', color: '#666' }}>October 2025</span>
                      </div>
                    </div>
                  </div>
                  <div style={{ fontSize: '16px', color: '#222', lineHeight: '1.6' }}>
                    Amazing unit! The place was clean and had everything I needed. The amenities were great—especially the complimentary bottled water, and all the small touches made the stay comfortable. Highly recommended!
                  </div>
                </div>
                  ...
                  <div style={{ display: 'flex', gap: '16px', marginBottom: '12px' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                        ...
                        <span style={{ fontSize: '14px', color: '#666' }}>September 2025</span>
                      </div>
                    </div>
                  </div>
                  <div style={{ fontSize: '16px', color: '#222', lineHeight: '1.6' }}>
                    Great location and excellent value for money. The host was very responsive and helpful throughout our stay. Would definitely stay here again!
                  </div>
                </div>
              </div>
            </div>
